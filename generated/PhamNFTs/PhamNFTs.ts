...
export class PhamNFTs extends ethereum.SmartContract {
  static bind(address: Address): PhamNFTs {
    return new PhamNFTs("PhamNFTs", address);
  }
  ...
  USER_OWNER_ROLE(): Bytes {
    let result = super.call(
      "USER_OWNER_ROLE",
      "USER_OWNER_ROLE():(bytes32)",
      []
    );
  ...
  try_USER_OWNER_ROLE(): ethereum.CallResult<Bytes> {
    let result = super.tryCall(
      "USER_OWNER_ROLE",
      "USER_OWNER_ROLE():(bytes32)",
      []
  ...
  setSupply(totalAmount: BigInt): boolean {
    let result = super.call("setSupply", "setSupply(uint256):(bool)", [
  ...
  try_setSupply(totalAmount: BigInt): ethereum.CallResult<boolean> {
    let result = super.tryCall("setSupply", "setSupply(uint256):(bool)", [
      ethereum.Value.fromUnsignedBigInt(totalAmount)
    ]);
    if (result.reverted) {
      return new ethereum.CallResult();
